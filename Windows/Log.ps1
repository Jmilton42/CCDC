...
if ($Env:PROCESSOR_ARCHITECTURE -eq "AMD64") {
    C:\Windows\System32\Sysmon64.exe -accepteula -i C:\Windows\System32\Sysmon.xml
    Write-Output "$Env:ComputerName [INFO] Sysmon64 installed and configured"
}
else {
    C:\Windows\System32\Sysmon.exe -accepteula -i C:\Windows\System32\Sysmon.xml
    Write-Output "$Env:ComputerName [INFO] Sysmon32 installed and configured"
}

msiexec.exe /i 'C:\Windows\System32\wazuh-agent-4.7.2-1.msi' /q WAZUH_MANAGER='IP.HERE' WAZUH_REGISTRATION_SERVER='IP.HERE'
net start WazuhSvc
