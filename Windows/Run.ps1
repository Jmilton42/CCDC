...
if (($Script -ne '') -and ($global:Sessions.Count -gt 0) -and ($Out -ne '')) {
    ...
    foreach ($Session in $global:Sessions) {
        ...
        if ($Script -match "Usr.ps1") {
            if ($P1 -eq '' -or $P2 -eq '') {
                Write-Host "[ERROR: $Script] Passwords are required" -ForegroundColor Red
                exit
            }
            $ScriptJob = Invoke-Command -FilePath $Script -Session $Session -ArgumentList $P1, $P2, $ExcludeU -AsJob
        }
        else {
